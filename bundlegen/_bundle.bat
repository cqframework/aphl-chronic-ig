...
IF EXIST "%input_cache_path%\%tooling_jar%" (
	ECHO running: JAVA -jar "%input_cache_path%\%tooling_jar%" -BundleResources -ptd="%sourcefiles_path%" -op="%bundlegen_path%" -v=r4 -e=json -bundleid=%1
	...
	JAVA -jar "..\%tooling_jar%" -BundleResources -ptd="%sourcefiles_path%" -op="%bundlegen_path%" -v=r4 -e=json -bundleid=%1
) ELSE (
